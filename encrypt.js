#!/usr/bin/env node
/**
 * Encrypts all photos in ./photos/ with AES-256-GCM (PBKDF2 key).
 * Output: data.enc.js  — commit this to the repo.
 * Gitignore: photos/   — raw photos never go to git.
 *
...
async function encryptFile(key, filePath) {
...
async function main() {
  ...
  for (const file of files) {
    photos[file] = await encryptFile(key, path.join(PHOTOS_DIR, file));
    console.log(`  ✓ ${file}`);
  }

  const payload = JSON.stringify({ salt: toB64(salt), photos });
  ...
  console.log('  2. Make sure photos/ is in .gitignore (already done)');
}
